...
const NEWS_CACHE_DURATION_MS = 60000; // 1 minute (60 seconds)
...
export async function GET(request: Request) {
  try {
    ...
    const now = Date.now();
    let allArticles: NewsItem[] = cachedNewsArticles || [];

    if (!cachedNewsArticles || (now - lastNewsFetchTime > NEWS_CACHE_DURATION_MS)) {
      ...
      xmlResults.forEach((xmlText, feedIndex) => {
        ...
        while ((match = itemRegex.exec(xmlText)) !== null) {
          ...
          if (lowerText.includes("fall") || lowerText.includes("drop") || lowerText.includes("slump") || lowerText.includes("decline") || lowerText.includes("losses") || lowerText.includes("down") || lowerText.includes("bearish") || lowerText.includes("plunge") || lowerText.includes("fell")) {
            impactScore = -4;
          } else if (lowerText.includes("rose") || lowerText.includes("surge") || lowerText.includes("growth") || lowerText.includes("gains") || lowerText.includes("up") || lowerText.includes("bullish") || lowerText.includes("higher") || lowerText.includes("jump")) {
            impactScore = 6;
          }
      ...
      cachedNewsArticles = parsedArticles;
